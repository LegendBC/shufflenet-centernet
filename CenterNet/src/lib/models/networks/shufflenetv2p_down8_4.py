...
class ShuffleNetV2(nn.Module):
    def __init__(self,heads, head_conv, n_class=1000, input_size=224, width_mult=1.):
        ...
        self.conv_final  = conv_1x1_bn(input_channel, self.stage_out_channels[-1])
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                  ...
                  #nn.ReLU(inplace=True),
                  DWconv(self.inplanes, head_conv, 1, 1),
    ...
    def forward(self, x):
        ...
        x = cem1_output+cem3_output
        #x = self._concat(cem1_output,cem3_output)
        x = self.cemse(x)
        ret = {}
